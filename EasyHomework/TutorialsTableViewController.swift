...
class TutorialsTableViewController: UITableViewController {
    ...
    var dictionary :[Int:Array<ScheduleRowContent>] = [0 : [ScheduleRowContent(identifier: "InstructionsCell")], 1 : [ScheduleRowContent(identifier: "TutorialCell")] ]
    ...
    override func unwindToViewController(sender: UIStoryboardSegue) {
        ...
        if (sender.identifier == "UnwindToPreviousVC") {
            let sourceVC = sender.source as! TutorialEditingTableViewController
            //get information from sourceVC.
            let cellContent = ScheduleRowContent(identifier: "TutorialCell")
            cellContent.toggle = true
            self.dictionary[1]?.insert(cellContent, at: 0)
            self.tableView.reloadData()
            self.title = "Tutorial Selected"
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if (section == 1) {
            let headerView = SectionHeaderView.construct("Tutorials", owner: tableView)
            return headerView
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if (segue.identifier == "ExistingTutorial") {
            segue.destination.title = "Tutorial"
        }
        
        if (segue.identifier == "NewTutorial") {
            segue.destination.title = "Create Tutorial"
        }
