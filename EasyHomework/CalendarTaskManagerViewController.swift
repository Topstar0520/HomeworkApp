//
//  CalendarTaskManagerViewController
//  EasyHomework
//
//  Created by Anthony Giugno on 2019-05-11.
...
class CalendarTaskManagerViewController: B4GradViewController, UITableViewDataSource, UITableViewDelegate, HomeworkTableViewCellDelegate, AVAudioPlayerDelegate, NotificationBannerViewDelegate, UITabBarDelegate {
    ...
    private var isComingFromCalendar        = true
    var tasksFromCalendar: [RLMTask] = []
    ...
    var activeTasks: [RLMTask] { //uncompleted tasks.  //used to be Results<RLMTask>
        if self.isComingFromCalendar {
            var filteredTasks = self.tasksFromCalendar.filter {$0.completed == false && $0.removed == false}
            filteredTasks.sort(by: { $0.dueDate?.compare($1.dueDate as? Date ?? Date(timeIntervalSinceNow: 99999)) == .orderedAscending })
        ...
        //        let allEvents   = realm.objects(RLMTask.self).toArray()
    ...
    var completedTodayTasks: [RLMTask]{ //Results<RLMTask> { //completed tasks.
        if self.isComingFromCalendar {
            var filteredTasks = self.tasksFromCalendar.filter {$0.completed == true && $0.removed == false}
            filteredTasks.sort(by: { $0.completionDate!.compare($1.completionDate as! Date) == .orderedDescending })
            return filteredTasks
        }
        ...
        return completedTodayTasks.toArray()
    }
    ...
    var extendedTasks: [RLMTask] { //extended tasks. //used to be Results<RLMTask>
        ...
        if self.isComingFromCalendar {
            //            let filteredTasks = self.tasksFromCalendar.filter {$0.dueDate?.compare(twoWeeksFromNow) == ComparisonResult.orderedDescending || $0.dueDate?.compare(twoWeeksFromNow) == ComparisonResult.orderedSame}
            //            return filteredTasks
            return []
        }
    ...
    override func viewDidLoad() {
        ...
        self.homeVC = self.calendarViewController.homeVC
        ...
        if self.isComingFromCalendar {
            self.view.backgroundColor = UIColor.init(hex: "1B1C1C")
                ...
                userQuery.findObjectsInBackground(block: {
                    ...
                    if user != nil {
                        if (user?.count)! > 0 {
                            ...
                            let strName = obj.object(forKey: "displayName") as? String ?? "Anonymous"
                            let alert = SCLAlertView()
                            _ = alert.addButton("Create account", target:self, selector:#selector(self.profileButtonTapped))
                            _ = alert.showSuccess("Congratulations", subTitle: "You Received a Referral Point from your friend \(strName)! \n Create an account to claim it", closeButtonTitle: "Do it later", timeout: nil, colorStyle: 0x007FC1, colorTextButton: 0xFFFFFF, circleIconImage: UIImage(named: "Premium"), animationStyle: .topToBottom)
                        }
                    }
                })
            }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if #available(iOS 11.0, *) {
            if self.tabBarController != nil{
                let nnHight = self.tableView.contentInset.bottom + self.tableView.contentInset.top + self.additionalSafeAreaInsets.top + self.additionalSafeAreaInsets.bottom + self.tabBarController!.tabBar.frame.size.height + self.navigationController!.navigationBar.frame.size.height + UIApplication.shared.statusBarFrame.size.height
                let nHeight = self.tableView.bounds.height - nnHight
                self.emptyHomescreenView.frame = CGRect(x: 0, y: 0, width: self.tableView.bounds.width, height: nHeight)
            }else{
                var height = self.tableView.bounds.height - (self.tableView.contentInset.bottom + self.tableView.contentInset.top + self.additionalSafeAreaInsets.top + self.additionalSafeAreaInsets.bottom + UIApplication.shared.statusBarFrame.size.height)
                if (self.navigationController != nil) {
                    height = height - self.navigationController!.navigationBar.frame.size.height
                }
                self.emptyHomescreenView.frame = CGRect(x: 0, y: 0, width: self.tableView.bounds.width, height: height)
            }
        } else {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if (self.updateLabelsTimer == nil && !self.isComingFromCalendar) {
            self.updateLabelsTimer = Timer.scheduledTimer(timeInterval: 30.0, target: self, selector: #selector(self.UpdateDateLabels), userInfo: nil, repeats: true)
            RunLoop.current.add(self.startingSoonTimer, forMode: RunLoopMode.commonModes)
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (task.dueDate != nil && Calendar.current.isDate(task.dueDate! as Date, inSameDayAs: self.selectedDate)) {
            cell.cardView.alpha = 1.0
        } else {
            cell.cardView.alpha = 0.4
        }
        ...
        for indexPath in indexPathsForVisibleCells! {
            if let hwCell = self.tableView.cellForRow(at: indexPath) as? HomeworkTableViewCell {
                if (hwCell.isSelected == false) {
                    var tasksInSection = self.tasks(inSection: indexPath.section)
                    if isComingFromCalendar {
                        tasksInSection  = self.tasksFromCalendar
                    }
                    //if indexPath.row < tasksInSection.count{
    ...
    func handleEmptyTableView() { //The placeholder shown when the Agenda has no tasks to show.
        ...
        if (self.isComingFromCalendar) {
            self.emptyHomescreenView.mainTitleLabel.text = "No Tasks Happening Today."
            self.emptyHomescreenView.descriptionLabel.text = "Why don't you add a Task that is due on this date?"
            self.emptyHomescreenView.arrowImageView.image = nil
        }
    ...
    func taskDeleted(_ task: RLMTask) {
        ...
        let indexPathHomeVC = self.homeVC.indexOfTask(task: task)
        
        if (indexPathHomeVC == nil) {
            self.homeVC.tableView.reloadData() //reload anyway just incase it is because UITabBarController released it from memory.
        }
        ...
        //Same for HomeVC (cellWasSelectedForHomeVC, self.homeVC, indexPathHomeVC)
        var cellWasSelectedForHomeVC = false
        if (indexPathHomeVC != nil && self.homeVC.tableView.cellForRow(at: indexPathHomeVC!)!.isSelected == true) {
            cellWasSelectedForHomeVC = true
        }
        ...
        //Update Calendar
        ///self.tasksFromCalendar.remove(at: indexPathForRow.row)
        self.calendarViewController.fetchTasks()
        ...
        if (indexPathHomeVC != nil) {
            ...
            self.homeVC.tableView.deleteRows(at: [indexPathHomeVC!], with: .left)
    ...
    func taskCompleted(_ task: RLMTask) {
        ...
        //Now, for HomeVC.
        self.handleTaskCompletedForHomeVC(task)
        
        self.calendarViewController.fetchTasks()
    }
    
    func handleTaskCompletedForHomeVC(_ task: RLMTask) {
        if let indexPathHomeVC = self.homeVC.indexOfTask(task: task) {
        
        let sectionHomeVC = indexPathHomeVC.section
        let indexRowHomeVC = indexPathHomeVC.row
        
        if (task.completed != true) {
            let indexPathForRow = IndexPath(row: indexRowHomeVC, section: sectionHomeVC)
            let cell = homeVC.tableView.cellForRow(at: indexPathForRow) as? HomeworkTableViewCell
            cell?.leadingCompletionConstraint.constant = 32
            strikeThroughLabel(cell?.titleLabel)
            strikeThroughLabel(cell?.courseLabel)
            strikeThroughLabel(cell?.dueDateLabel)
    ...
    func moveTask(_ cell: HomeworkTableViewCell, _ task: RLMTask) {
        ...
        var indexPathForHomeVC = self.homeVC.indexOfTask(task: task)
        var cellForHomeVC: HomeworkTableViewCell?
        if (indexPathForHomeVC != nil) {
            cellForHomeVC = self.homeVC.tableView.cellForRow(at: indexPathForHomeVC!) as! HomeworkTableViewCell
        }
        ...
        //Same for HomeVC. (Use self.homeVC and indexPathForHomeVC)
        //Add Completed Today Section if needed.
        if (cellForHomeVC != nil) {
        self.homeVC.tableView.beginUpdates()
        ...
        //Same with HomeVC. (Use self.homeVC and indexPathForHomeVC)
        if (cellForHomeVC != nil) {
        self.homeVC.tableView.beginUpdates()
        if (task.dueDate?.overScopeThreshold(task: task) == true && self.homeVC.extendedTasks.count == 0) {
        ...
        var newIndexPathForHomeVC = self.homeVC.indexOfTask(task: task)
        ...
        //Same for HomeVC. (Use self.homeVC, indexPathForHomeVC, newIndexPathForHomeVC, & cellForHomeVC.)
        if (newIndexPathForHomeVC != nil) {
        UIView.animate(withDuration: 0.27, delay: 0.0, options: [], animations: {
            self.homeVC.tableView.beginUpdates()
            self.homeVC.tableView.moveRow(at: indexPathForHomeVC!, to: newIndexPathForHomeVC!)
            if (task.completed == true) {
                cellForHomeVC!.titleLabel.textColor = self.FADED_BLACK_COLOR
                cellForHomeVC!.courseLabel.textColor = self.FADED_BLACK_COLOR
                cellForHomeVC!.dueDateLabel.textColor = self.FADED_BLACK_COLOR
                cellForHomeVC!.dateLabel.textColor = self.FADED_BLACK_COLOR
                cellForHomeVC!.repeatsImageView.image = #imageLiteral(resourceName: "Grey Repeats")
            }
            self.homeVC.tableView.endUpdates()
        }, completion: nil)
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let index = self.tableView.indexPathForSelectedRow {
            ...
            cellEditingVC.helperObject = CalendarCellEditingHelperObject(cellEditingTVC: cellEditingVC, task: task, taskManagerVC: self)
    ...
    @IBAction func addTaskButtonTapped(_ sender: Any) {
        ...
        newTask.dueDate = self.selectedDate.convertToLatestPossibleTimeOfDay() as NSDate
        
        cellEditingVC.helperObject = CalendarCellEditingHelperObject(cellEditingTVC: cellEditingVC, task: newTask, taskManagerVC: self)
