...
class HomeworkCellEditingHelperObject: NSObject, CellEditingProtocol {
    ...
    init(cellEditingTVC: CellEditingTableViewController, task: RLMTask, taskManagerVC: UIViewController?, homeVC: HomeworkViewController?) {
        ...
        if (self.task.dueDate != nil) { //Also check at setupForCreateMode() in CellEditingTVC if modifying anything here.
            ...
            self.dictionary[4] = [ScheduleRowContent(identifier: "ReminderCell")] //4
            ...
            self.dictionary[4] = [ScheduleRowContent(identifier: "ReminderCell")]
            self.dictionary[3] = [ScheduleRowContent(identifier: "NoteCell")]
        ...
        if let remindersCell = cell as? ReminderTitleCell {
            ...
            /*if (self.task.dueDate == nil) {
                let alertViewController = UIAlertController(title: "Oops..",
                                                            message: "Set a date before adding reminders.",
                                                            preferredStyle: .alert)
                let okButton = UIAlertAction(title: "OK", style: .default, handler: { (action) -> Void in
                    alertViewController.dismiss(animated: true, completion: nil)
                })
                alertViewController.addAction(okButton)
                self.cellEditingTVC.present(alertViewController, animated: true, completion: nil)
            }*/
        }
