//
//  CalendarCellEditingHelperObject.swift
//  B4Grad
//
//  Created by Anthony Giugno on 2019-07-11.
...
class CalendarCellEditingHelperObject: NSObject, CellEditingProtocol {
    ...
    init(cellEditingTVC: CellEditingTableViewController, task: RLMTask, taskManagerVC: UIViewController?) {
        super.init()
        ...
        //Set homeVC dynamically.
        let tabBarController = taskManagerVC?.splitViewController!.viewControllers[0] as! UITabBarController
        for vc in tabBarController.viewControllers! {
            let navController = vc as? UINavigationController
            //print(navController?.topViewController?.classForCoder)
            if navController?.viewControllers[0] is HomeworkViewController {
                self.homeVC = navController?.viewControllers[0] as! HomeworkViewController
                break
            }
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (cell?.reuseIdentifier == "CreateCell") {
            ...
            //Add self.task to CalendarTaskManagerVC.
            let calendarTaskManagerVC = self.taskManagerVC as! CalendarTaskManagerViewController
            if (self.task.dueDate != nil && calendarTaskManagerVC.tasksFromCalendar.count == 0 && Calendar.current.isDate(self.task!.dueDate as! Date, inSameDayAs: calendarTaskManagerVC.selectedDate)) {
                calendarTaskManagerVC.tasksFromCalendar.append(self.task)
            } else if (self.task.dueDate != nil && Calendar.current.isDate(self.task!.dueDate as! Date, inSameDayAs: calendarTaskManagerVC.selectedDate)) {
                for (index, task) in calendarTaskManagerVC.tasksFromCalendar.enumerated() {
                    if ((task.dueDate as! Date) > (self.task.dueDate as! Date) || (index+1) == calendarTaskManagerVC.tasksFromCalendar.count) {
                        calendarTaskManagerVC.tasksFromCalendar.insert(self.task, at: index)
                        break
                    }
                }
            }
            ...
            self.cellEditingTVC.dismiss(animated: true, completion: {
                UIView.animate(withDuration: 0.1, animations: { self.homeVC?.emptyHomescreenView.alpha = 0; calendarTaskManagerVC.emptyHomescreenView.alpha = 0 })
                ...
                if (calendarTaskManagerIndexPathOfTask != nil) {
    ...
    func requestReviewIfAllowed() {
        //let askedForReview = UserDefaults.standard.bool(forKey: "AskedForReview") //defaults to false
        //if (askedForReview == true) { return }
        let now = Date()
        ...
        if (tasksCount < 3 && tasksCount > 1) { //> 3
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        //Tapping Done (Return) button causes newline if the caret is not at the end of the string, this is a workaround for that.
    ...
    func textFieldEdited(sender: UITextView) {
        ...
        //update cell in CalendarTaskManager
        let calendarTaskManagerVC = self.taskManagerVC as! CalendarTaskManagerViewController
        if (calendarTaskManagerVC.lastSelectedRowIndexPath != nil) {
            let hwCellInCalendarTaskManagerVC = calendarTaskManagerVC.tableView.cellForRow(at: calendarTaskManagerVC.lastSelectedRowIndexPath!) as? HomeworkTableViewCell
            calendarTaskManagerVC.tableView.beginUpdates()
            hwCellInCalendarTaskManagerVC?.titleLabel.attributedText = NSAttributedString(string: titleString, attributes: hwCellInCalendarTaskManagerVC?.titleLabel.attributedText?.attributes(at: 0, effectiveRange: nil))
            calendarTaskManagerVC.tableView.endUpdates()
            calendarTaskManagerVC.tableView.selectRow(at: calendarTaskManagerVC.lastSelectedRowIndexPath!, animated: false, scrollPosition: .none)
        }
