...
extension String {
    ...
    func convertToDateWith(format:String, setUTCTimeZone:Bool) -> Date {
...
extension NSDate {
    ...
    func dayNumberOfMonth() -> Int? {
        return Calendar.current.dateComponents([.day], from: self as Date).day
    }
...
extension Date {
    ...
    func addComponent(component:Calendar.Component, withValue count:Int) -> Date {
        let calendar    = Calendar.current
        let newDate     = calendar.date(byAdding: component, value: count, to: self)
        return newDate!
    }

    func isWeeklyDateOf(inputDate:Date) -> Bool {
        var myDate      = inputDate
        var returnValue = false

        if self.compare(myDate) == ComparisonResult.orderedSame {
            returnValue = true
        }else{
            while (self.compare(myDate) == ComparisonResult.orderedDescending || self.compare(myDate) == ComparisonResult.orderedSame){
                if self.compare(myDate) == ComparisonResult.orderedSame {
                    returnValue = true
                    break
                }else{
                    myDate = myDate.addComponent(component: .weekOfMonth, withValue: 1)
                }
            }
        }
        return returnValue
    ...
    func isBiWeeklyDateOf(inputDate:Date) -> Bool {
        var myDate      = inputDate
        var returnValue = false
        ...
        if self.compare(myDate) == ComparisonResult.orderedSame {
            ...
            while (self.compare(myDate) == ComparisonResult.orderedDescending || self.compare(myDate) == ComparisonResult.orderedSame){
                if self.compare(myDate) == ComparisonResult.orderedSame {
                    ...
                    myDate = myDate.addComponent(component: .weekOfMonth, withValue: 2)
                }
            }
        }
        return returnValue
    ...
    func isMonthlyDateOf(inputDate:Date) -> Bool {
        var myDate      = inputDate
        var returnValue = false
        ...
        while (self.compare(myDate) == ComparisonResult.orderedDescending || self.compare(myDate) == ComparisonResult.orderedSame){
            if self.compare(myDate) == ComparisonResult.orderedSame {
                ...
                myDate = myDate.addComponent(component: .month, withValue: 1)
            }
        }
    ...
    func stringWithFormat(format:String, setUTCTimeZone:Bool) -> String {
        let formatter 	        = DateFormatter.init()
        formatter.dateFormat    = format
        if setUTCTimeZone {
            formatter.timeZone  = TimeZone.init(secondsFromGMT: 0)
        ...
        let stringType          = formatter.string(from: self)
...
extension UIColor {
    ...
    class func unSelectedBorderColor() -> UIColor {
        return UIColor(red: 1.0, green: 1.0, blue: 1.0, alpha: 0.1)
    }
    class func cellBlueColor() -> UIColor {
        return UIColor(red: 43/255, green: 132/255, blue: 210/255, alpha: 1)
    }

    class func cellGreenColor() -> UIColor {
        return UIColor(red: 44/255, green: 197/255, blue: 94/255, alpha: 1)
    }

    class func cellYellowColor() -> UIColor {
        return UIColor(red: 237/255, green: 186/255, blue: 16/255, alpha: 1)
    }

    class func cellOrangeColor() -> UIColor {
        return UIColor(red: 222/255, green: 106/255, blue: 27/255, alpha: 1)
    }
    class func cellredColor() -> UIColor {
        return UIColor(red: 223/255, green: 52/255, blue: 46/255, alpha: 1)
    }
...
//A student dev added this SwifterSwift thing below, should probably be removed and any code using it replaced with vanilla iOS versions. It is unnecessary. It is used for the instructor feature.
