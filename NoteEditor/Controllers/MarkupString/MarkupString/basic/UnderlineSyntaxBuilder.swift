//
//  UnderlineSyntaxBuilder.instance.swift
//  Note Editor
//
...
class UnderlineSyntaxBuilder: MarkupSyntaxBuilder {
    ...
    func addMarkupSyntax(in range: NSRange, with string: NSAttributedString, optional value: Any?) -> (NSRange, NSAttributedString) {
        if range.length == 0 {
            ...
            let underlineSign = NSAttributedString(string: "_")
            ...
            for matchRange in matchRanges {
                if matchRange.length > 0 {
                    str.insert(underlineSign, at: matchRange.location - range.location + shiftLen)
                    str.insert(underlineSign, at: matchRange.location - range.location + matchRange.length + shiftLen + underlineSign.length)
                    shiftLen += 2*underlineSign.length
                }
            }
