//
//  HighlightSyntaxBuilder.instance.swift
//  Note Editor
//
...
class HighlightSyntaxBuilder: MarkupSyntaxBuilder {

    static let instance = HighlightSyntaxBuilder()
    ...
    func stylizeSyntaxElements(in range: NSRange, with string: NSMutableAttributedString) -> NSRange {
        ...
        regex().enumerateMatches(in: string.string , options: [], range: range, using: { (match, flags, stop) in
            ...
            if let startTagRange = match?.range(at: 1), let _ = match?.range(at: 2), let endTagRange = match?.range(at: 3) {
                //UIColor.white
                //ThemeCenter.theme.highlightTextColor
                string.addAttributes([NSAttributedString.Key.font : UIFont.systemFont(ofSize: 17), NSAttributedString.Key.backgroundColor: ThemeCenter.theme.highlightColor, NSAttributedString.Key.foregroundColor : ThemeCenter.theme.highlightTextColor,
                                      NSAttributedString.Key.backgroundRenderingMode: BackgroundRenderingMode.HighLight], range: NSMakeRange(startTagRange.location, NSMaxRange(endTagRange) - startTagRange.location))
    ...
    func addMarkupSyntax(in range: NSRange, with string: NSAttributedString, optional value: Any?) -> (NSRange, NSAttributedString) {
        if range.length == 0 {
            ...
            let doubleColons = NSAttributedString(string: "::")
            ...
            for matchRange in matchRanges {
                if matchRange.length > 0 {
                    str.insert(doubleColons, at: matchRange.location - range.location + shiftLen)
                    str.insert(doubleColons, at: matchRange.location - range.location + matchRange.length + shiftLen + doubleColons.length)
                    shiftLen += 2*doubleColons.length
                }
            }
